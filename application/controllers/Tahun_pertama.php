...
class Tahun_pertama extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Tahun_pertama_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tahun_pertama/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tahun_pertama/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tahun_pertama/index.html';
            $config['first_url'] = base_url() . 'tahun_pertama/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tahun_pertama_model->total_rows($q);
        $tahun_pertama = $this->Tahun_pertama_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'tahun_pertama_data' => $tahun_pertama,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('tahun_pertama/tbl_tahun_pertama_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Tahun_pertama_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'des' => $row->des,
            ...
            $this->load->view('tahun_pertama/tbl_tahun_pertama_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tahun_pertama'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('tahun_pertama/create_action'),
        ...
        $this->load->view('tahun_pertama/tbl_tahun_pertama_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_pertama_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tahun_pertama/update_action'),
    ...
    public function update_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_pertama_model->update($this->input->post('id_tahun_pertama', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tahun_pertama_model->delete($id);
...
/* End of file Tahun_pertama.php */
/* Location: ./application/controllers/Tahun_pertama.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2018-09-21 05:49:31 */
/* http://harviacode.com */