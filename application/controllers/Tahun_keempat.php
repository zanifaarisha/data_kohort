...
class Tahun_keempat extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Tahun_keempat_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tahun_keempat/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tahun_keempat/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tahun_keempat/index.html';
            $config['first_url'] = base_url() . 'tahun_keempat/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tahun_keempat_model->total_rows($q);
        $tahun_keempat = $this->Tahun_keempat_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'tahun_keempat_data' => $tahun_keempat,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('tahun_keempat/tbl_tahun_keempat_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Tahun_keempat_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'des' => $row->des,
            ...
            $this->load->view('tahun_keempat/tbl_tahun_keempat_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tahun_keempat'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('tahun_keempat/create_action'),
        ...
        $this->load->view('tahun_keempat/tbl_tahun_keempat_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_keempat_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tahun_keempat/update_action'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->update($this->input->post('id_tahun_keempat', TRUE));
        } else {
            $data = array(
		'jan' => $this->input->post('jan',TRUE),
		'feb' => $this->input->post('feb',TRUE),
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_keempat_model->update($this->input->post('id_tahun_keempat', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tahun_keempat_model->delete($id);
	...
	$this->form_validation->set_rules('id_tahun_keempat', 'id_tahun_keempat', 'trim');
	$this->form_validation->set_error_delimiters('<span class="text-danger">', '</span>');
    }

}

/* End of file Tahun_keempat.php */
/* Location: ./application/controllers/Tahun_keempat.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2018-09-21 06:33:59 */
/* http://harviacode.com */