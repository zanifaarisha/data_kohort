...
class Tahun_ketiga extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Tahun_ketiga_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tahun_ketiga/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tahun_ketiga/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tahun_ketiga/index.html';
            $config['first_url'] = base_url() . 'tahun_ketiga/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tahun_ketiga_model->total_rows($q);
        $tahun_ketiga = $this->Tahun_ketiga_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'tahun_ketiga_data' => $tahun_ketiga,
            'q' => $q,
    ...
    {
        $row = $this->Tahun_ketiga_model->get_by_id($id);
        if ($row) {
            $data = array(
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('tahun_ketiga/create_action'),
    ...
    public function create_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_ketiga_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tahun_ketiga/update_action'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->update($this->input->post('id_tahun_ketiga', TRUE));
        } else {
            $data = array(
		'jan' => $this->input->post('jan',TRUE),
		'feb' => $this->input->post('feb',TRUE),
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_ketiga_model->update($this->input->post('id_tahun_ketiga', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tahun_ketiga_model->delete($id);
	...
	$this->form_validation->set_rules('id_tahun_ketiga', 'id_tahun_ketiga', 'trim');
	$this->form_validation->set_error_delimiters('<span class="text-danger">', '</span>');
    }

}

/* End of file Tahun_ketiga.php */
/* Location: ./application/controllers/Tahun_ketiga.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2018-09-21 06:33:43 */
/* http://harviacode.com */