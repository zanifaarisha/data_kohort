...
class Tahun_pertama_bayi extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Tahun_pertama_bayi_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tahun_pertama_bayi/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tahun_pertama_bayi/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tahun_pertama_bayi/index.html';
            $config['first_url'] = base_url() . 'tahun_pertama_bayi/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tahun_pertama_bayi_model->total_rows($q);
        $tahun_pertama_bayi = $this->Tahun_pertama_bayi_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'tahun_pertama_bayi_data' => $tahun_pertama_bayi,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('tahun_pertama_bayi/tbl_tahun_pertama_bayi_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Tahun_pertama_bayi_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'des' => $row->des,
            ...
            $this->load->view('tahun_pertama_bayi/tbl_tahun_pertama_bayi_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tahun_pertama_bayi'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('tahun_pertama_bayi/create_action'),
        ...
        $this->load->view('tahun_pertama_bayi/tbl_tahun_pertama_bayi_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_pertama_bayi_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tahun_pertama_bayi/update_action'),
    ...
    public function update_action() 
    {
		...
		'des' => $this->input->post('des',TRUE),
            ...
            $this->Tahun_pertama_bayi_model->update($this->input->post('id_tahun_pertama', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tahun_pertama_bayi_model->delete($id);
...
/* End of file Tahun_pertama_bayi.php */
/* Location: ./application/controllers/Tahun_pertama_bayi.php */
